						...
						if (e != null) {
							...
							if(!results.isEmpty()) {
								...
								for(ParseObject po : (List<ParseObject>)results) {
									poi.routes.add(new ClimbRoute(po.getString("name"),po.getString("info"),po.getString("rank"),ClimbXpertData.rid,(float)po.getDouble("azimuth"),(float)po.getDouble("tilt"), (int)po.getInt("pid")));
								}
							}
						}
					}
